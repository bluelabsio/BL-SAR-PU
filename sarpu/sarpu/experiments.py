...
def train_and_evaluate(data_folder, results_folder, data_name, labeling_model, labeling, partition, settings, pu_method, rerun_experiments=False, folder_name = ''):
    ...
    """
    out_dir = experiment_method_result_folder_path(results_folder, data_name, labeling_model, labeling, partition, settings, pu_method, folder_name)
    os.makedirs(out_dir, exist_ok=True)
    print('OUT', experiment_results_path(results_folder, data_name, labeling_model, labeling, partition, settings, pu_method, folder_name))
    if (
        rerun_experiments
        or not (
            os.path.exists(experiment_results_path(results_folder, data_name, labeling_model, labeling, partition, settings, pu_method, folder_name)) # results already exist
        ...
        out_info = experiment_info_path(results_folder, data_name, labeling_model, labeling, partition, settings, pu_method, folder_name)
        print('OUT', out_info)
        with open(out_info, "w+") as out_info_file:
            out_info_file.write("\n".join([k+"\t"+str(v) for k,v in sorted(info.items())]))

        out_classifier = experiment_classifier_path(results_folder, data_name, labeling_model, labeling, partition, settings, pu_method, folder_name)
        with open(out_classifier, "wb+") as out_classifier_file:
            pickle.dump(f_model, out_classifier_file)

        out_propensity_model = experiment_propensity_model_path(results_folder, data_name, labeling_model, labeling, partition, settings, pu_method, folder_name)
        ...
        out_results = experiment_results_path(results_folder, data_name, labeling_model, labeling, partition, settings, pu_method, folder_name)
...
def copy_if_possible(results_folder, data_name, labeling_model, labeling, partition, settings, pu_method):
    ...
    if pu_method!="supervised":
        ...
        try:
            complete_result_path = next(glob.iglob(experiment_method_result_path_nolabel(results_folder, data_name, partition, settings, pu_method, folder_name)))
...
def parse_model(string):
    model, penalty_val, penalty_type = string.split('-')
    return {
        "lr": LogisticRegressionPU(C = float(penalty_val), penalty = penalty_type),
        "sgd": SGDClassifierPU(alpha = float(penalty_val), penalty = penalty_type),
    }[model]
...
#     s_results = evaluate_classification(s, e_pred) ## BGB Note: We can't evaluate these measures since we lack ground-truth positive class labels
    e_results = evaluate_propensity_scores(e, e_pred)

    return {
        **{"f_" + k: v for k, v in f_results.items()},
#         **{"s_" + k: v for k, v in s_results.items()}, ## BGB Note: We can't evaluate these measures since we lack ground-truth positive class labels
